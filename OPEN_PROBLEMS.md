...
| L4.7 | Kiosk HTML gate-card section emitted a sentence that falsely concretized `:RemoteBiometricIdentificationProcess` for the verification kiosk. Root: `gate2_ok = intended_use_ok` at `run_pipeline.py:806` (variable name suggested Gate 2 typed-content satisfaction; the bound ASK is documentary-traceability per `reasoning/check_intended_use.sparql:5-12`); plus `or "RemoteBiometricIdentificationProcess"` Python literal fallback at `run_pipeline.py:816` (Sentinel-shaped literal, forbidden per `output_manifest_v2.yaml` `forbidden_source_patterns`). When `gate2_ok` was TRUE on the kiosk and the typed-evidence label was empty, the literal fired, producing "An Intended Use Specification prescribes the system for RemoteBiometricIdentificationProcess" — false against the loaded TTL. Output-discipline defect; reasoner output was correct throughout. | `run_pipeline.py:806` (variable binding), `run_pipeline.py:815-817` (three Python literal fallbacks), `run_pipeline.py:849` (HTML sentence template) | DONE (2026-05-11) | (1) Rebind `gate2_ok` to `bool(gate_evidence["gate2"]["process_type_uri"])` so the HTML affirmative branch only fires when typed evidence exists. (2) Replace the three `or "<class IRI>"` Python literal fallbacks with non-concretizing placeholders. (3) Add regression test `test_kiosk_html_no_false_concretization.py` asserting kiosk HTML does not emit the Gate 2 affirmative phrase. | CLOSED: Kiosk HTML produces no Gate 2 affirmative sentence (negative branch fires under typed-evidence absence); `test_kiosk_html_no_false_concretization.py` PASS; `output_manifest_v2.yaml` forbidden-pattern grep no longer matches `\bor "BiometricIdentificationCapability"\b` or `\bor "RemoteBiometricIdentificationProcess"\b` in `run_pipeline.py`; 5/5 fixture pipeline runs continue passing per prior baseline. |
