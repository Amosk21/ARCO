...
The short version. The reasoning input is nine files under `03_TECHNICAL_CORE/ontology/`: the core and governance ontologies, a fixture (input facts), the pinned BFO 2020 file, three slim modules for RO, IAO, and CCO, and `catalog-v001.xml`. The ontology IRIs use a domain that is intentionally not registered on the web, so nothing resolves online; the catalog maps every import to its pinned local file. Tools that read OASIS XML catalogs (Protege, ROBOT, and other OWL API loaders) pick it up automatically when it sits next to the file being opened. A load without the catalog fails immediately and loudly at the first import. That is by design: the alternative, live web IRIs, could silently fetch a different upstream version than the one the classification was verified against.
...
- **Command line.** The recipe includes a small standalone script (rdflib assembly, then Pellet or HermiT through owlready2, no ARCO code imported) and a two-command ROBOT variant. The ROBOT variant was executed cold from a fresh directory containing only the published files, with imports resolved purely through the catalog; the log lives with the verification program's audit records.
