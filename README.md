...
The underlying problem is older than AI regulation and better documented. A decade after the Basel Committee published its principles for risk data aggregation, its own 2023 review found two of thirty-one globally systemic banks fully compliant, and its root-cause list named inconsistent, unintegrated and imprecise data dictionaries, data models and definitions. Nobody in that report is selling ontology.
...
Underneath, ARCO runs on a formal foundation called BFO. It's an international standard (ISO/IEC 21838-2), and in January 2024 the Department of Defense Chief Digital and AI Officer and the Intelligence Community Chief Data Officer directed it as one of three baselines for their own formal ontology work. What that buys you in plain terms: the words mean one fixed thing every time, so two descriptions that commit to the same thing in different wording get the same answer.

ARCO isn't tied to this one law. What it checks is always the same three things: what a system can do, what it's for, and who it affects. Those are the three the EU AI Act's high-risk conditions turn on, and the machine re-aims at another category by swapping which capability and which process it looks for. I've built it out for two of the EU's high-risk categories so far.
