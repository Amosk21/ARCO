...
1. **Formal entailment-preservation guarantee.** BOT is a syntactic locality module variant (Cuenca Grau, Horrocks, Kazakov, Sattler 2007/2008): for any axiom α whose signature is contained in the seed signature Σ, the extracted module entails α if the full upstream ontology does. This includes property characteristics (`FunctionalProperty`, `Transitive`, `Symmetric`), property chain axioms, inverse-of axioms, and `rdfs:domain` / `rdfs:range`. ARCO's gate axioms depend on these — particularly anonymous inverse property expressions on `iao:0000136` — so this is the strict property the project needs.
