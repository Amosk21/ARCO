...
7. **~~HighRiskSystem vs AnnexIII category gate asymmetry.~~ FIXED.** `HighRiskSystem` axiom was missing `SystemComponent` in its `has_part` chain, contradicting CLAUDE.md's documented intent (`HighRiskSystem ≡ System ∩ has_part some (SystemComponent ∩ has_disposition some AnnexIIITriggeringCapability)`). A bare `bfo:Object` with a triggering capability would be entailed as `HighRiskSystem` but not as any specific Annex III category. **Fixed** — `HighRiskSystem` now requires `SystemComponent`, matching the category-specific classes and the documented architecture.
...
5. **owlrl disjointness detection mechanism.** owlrl handles `owl:disjointWith` via rule cax-dw by populating `closure.error_messages` — it does NOT entail `owl:Nothing` membership. Empirically verified: a deliberate disjointness violation (individual typed as both A and B with `owl:disjointWith`) fires `error_messages` but produces no `owl:Nothing` triple. The pipeline's `error_messages` check is the correct and only mechanism.

6. **Anonymous inverse property in equivalentClass.** Gate 2's axiom structure uses `[ owl:inverseOf iao:0000136 ]` as an anonymous property expression. Empirically verified to produce identical entailment results as `DeductiveClosure.expand()` under owlrl 7.1.4 (same triple counts, same classifications). Not a current fragility, but the version pin is still warranted as a general stability measure.
