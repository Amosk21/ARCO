...
- (\*) **Interest modeling for capability accountability** — ARCO follows the capability framing "disposition + interest of an organism or group"; ARCO models the disposition side only currently. Decision pending across three options (CCO relation only / Quality class per abi production / hybrid). `M-Capability-1 / L2.6`.
