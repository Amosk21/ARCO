...
- (\*) **Interest modeling for capability accountability** — the canonical capability framing in the applied-ontology literature is "disposition + interest of an organism or group"; ARCO models the disposition side only currently. Decision pending across three options (CCO relation only / Quality class per abi production / hybrid). `M-Capability-1 / L2.6`.
...
- (\*) **`:CapabilityDisposition` rename to `:Capability`** — compositional-naming discipline fix (a composed name requires its anchor terms); mechanical PR. `M-NameDiscipline-1 / L2.8`.
