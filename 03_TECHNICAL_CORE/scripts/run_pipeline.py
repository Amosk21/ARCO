...
def main() -> None:
    ...
    # Determination bearer chain — runtime emission, not a fixture commitment.
    # Per Ceusters/Smith Aboutness §5: every ICE is concretized in some SDC quality
    # of a real bearer. The certificate file ARCO emits is the bearer; its character
    # pattern is the SDC. We assert this at runtime so the determination ICE is not
    # free-floating in the audit-side graph. Uses BFO 2020 vocabulary only
    # (bfo:0000058 = concretizes, SDC -> GDC).
    # See: runs/loop/2026-05-09_beverley-research/design_memo_named-evidence-bearing-particulars.md
    #      runs/loop/2026-05-09_beverley-research/modeling_decisions_queue.md (Q2)
    cert_path = OUTPUT_DIR / "certificate.txt"
    run_id_iso = determination_packet["run_id"]
    bearer_token = run_id_iso.replace(":", "").replace(".", "").replace("+", "p").replace("-", "")
    bearer_local = f"Certificate_TextPattern_{bearer_token}"
    bearer_iri = f"{ARCO_NS}{bearer_local}"
    cert_uri = cert_path.resolve().as_uri()
    determination_iri = determination_packet["determination_node_uri"]

    determination_packet["bearer"] = {
        "bearer_uri": bearer_iri,
        "bearer_kind": "bfo:0000019",
        "bearer_kind_label": "quality (SDC)",
        "concretizing_relation": "bfo:0000058",
        "concretizing_relation_label": "concretizes",
        "concretizes_uri": determination_iri,
        "physical_artifact_uri": cert_uri,
        "physical_artifact_path": str(_repo_relative(cert_path)),
        "rationale": "Per Ceusters/Smith Aboutness §5; BFO 2.0 §3.8 Relation of Concretization. Asserts the determination ICE has a real SDC bearer (the text pattern of the emitted certificate file).",
    }
    ...
    # determination_bearer.ttl — the bearer chain as a loadable TTL fragment so an
    # auditor or downstream tool can inspect the concretization in graph form.
    bearer_ttl = (
        f"@prefix : <{ARCO_NS}> .\n"
        f"@prefix bfo: <http://purl.obolibrary.org/obo/BFO_> .\n"
        f"@prefix cco: <http://www.ontologyrepository.com/CommonCoreOntologies/> .\n"
        f"@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> .\n"
        f"\n"
        f"# Runtime emission. The pattern of characters in certificate.txt for this run\n"
        f"# is an SDC quality that concretizes the HighRisk_Determination_001 ICE\n"
        f"# (Ceusters/Smith Aboutness §5; BFO 2.0 §3.8). Generated by run_pipeline.py;\n"
        f"# do not hand-edit. See design_memo_named-evidence-bearing-particulars.md.\n"
        f"\n"
        f":{bearer_local}\n"
        f"    a bfo:0000019 ;\n"
        f'    rdfs:label "Certificate text pattern for run {run_id_iso}" ;\n'
        f"    bfo:0000058 <{determination_iri}> ;\n"
        f'    rdfs:comment "Bearer SDC for the HighRisk determination ICE emitted by this run. The character pattern in certificate.txt at the path below specifically depends on the file as material entity; the determination ICE generically depends on the same." .\n'
    )
    (OUTPUT_DIR / "determination_bearer.ttl").write_text(bearer_ttl, encoding="utf-8")
