...
`:UseScenarioSpecification rdfs:subClassOf cco:DesignativeInformationContentEntity`. The `cco:designates` filler is the role universal class IRI, not a role-token individual. `cco:designates` carries no range declaration of its own in the pinned CCO; the `bfo:0000001` (Entity) range is inherited from its parent `cco:is_about`. Using the class IRI as the filler makes it an OWL-punned individual: legal under OWL 2 DL (the loaded ontology passes the DL profile check), consumed by the reasoner in its individual interpretation only, and not a use CCO itself documents (CCO's own examples all designate particulars). Whether this widened reading is fully licensed is the same open modeling question flagged below. Class-level designation is the load-bearing pattern: the spec designates the regulated role category by name, without instantiating a role-bearer at design time.
...
- **FORWARD-INCOMPATIBLE WITH THE BETTER-EVIDENCED CASE. Added 2026-07-27, reproduced by execution.** The `owl:hasValue` encoding and a properly borne role particular are mutually exclusive. A reviewer who obtains real source warrant for an affected person, mints the role token correctly, and points `cco:designates` at it **loses the Annex III 1(a) entailment and additionally trips a SHACL violation.**
  ...
  Executed against the Sentinel fixture with one line changed, `cco:designates :NaturalPersonRole` replaced by `cco:designates :Sentinel_AffectedPersonRole_001`, where that individual is typed `:NaturalPersonRole` and inheres in a `cco:Person` via `ro:0000052` (the property ARCO actually commits, bound as `rdfs:subPropertyOf bfo:0000197` per §3.8):
  ...
  **Bound on this finding.** One role-particular shape was tested (typed to the role universal, inhering in a `cco:Person` via `ro:0000052`). OWL-RL only; HermiT was not run against the variant. All three layers were confirmed to reject it by execution: the entailment is absent from the closure, SHACL raises `HasValueConstraintComponent`, and the intended-use audit query returns `False`. Other bearer relations, other role shapes, and the 5(b) branch were not tested. The probe fixture is committed at [probe_gate3_role_particular.ttl](03_TECHNICAL_CORE/ontology/probes/probe_gate3_role_particular.ttl), so the run is reproducible from a clone: `python 03_TECHNICAL_CORE/scripts/run_pipeline.py --instances 03_TECHNICAL_CORE/ontology/probes/probe_gate3_role_particular.ttl`.
...
A second, narrower simplification sits inside this gate: for a model-driven biometric module, the in-repo Sentinel fixture types the bearer as `:HardwareComponent` only, but a strict realist reading would locate the disposition on the *amalgam* of hardware plus the concretized model artifact running on it. The 2024 Capabilities paper treats software qua pattern as a generically dependent continuant, not itself a capable continuant. Its amalgam passage states that generically dependent continuants such as software tools "cannot bear capabilities," and its BFO elucidations supply the mechanism separately, that a pattern "exists only if it is concretized in some continuant bearer." (An earlier revision attributed that second phrase to the amalgam discussion itself, and narrowed the concretizer to a quality. The concretizer is a process or a specifically dependent continuant, of which quality is one branch alongside realizable entity, never the material entity.) So capabilities require a material bearer in which the pattern is concretized. ARCO's classification result does not depend on which of these two bearer choices is taken — Gate 1 is satisfied as long as some component bearer instantiates a triggering-capability disposition — so the simplification is documented here rather than rebuilt as a structural change.
...
- **Negative test infrastructure is incomplete.** Gate-removal regression tests ([test_gate_removal.py](03_TECHNICAL_CORE/scripts/test_gate_removal.py)) verify each gate is independently necessary for both modeled Annex III categories (1(a) Sentinel and 5(b) CreditScorer) by mutating axioms and confirming entailment breaks. Symmetric coverage across the two categories was added 2026-05-14. Adversarial-mechanism tests ([test_adversarial_mechanism.py](03_TECHNICAL_CORE/scripts/test_adversarial_mechanism.py)) verify three cases: DecoySystem_001 classifies via `owl:equivalentClass` on the Annex III 1(a) branch (not direct IRI assertion); WeirdCalcSystem_001 classifies via `owl:equivalentClass` on the 5(b) branch with cross-category isolation preserved (added 2026-05-20); and GhostSystem_001 classifies via blank-node `owl:someValuesFrom` (not a named individual). **CORRECTED 2026-07-27.** This bullet previously said no parameterized negative-case harness existed, because the pipeline loaded all TTL files into a single graph and a negative fixture would contaminate positive cases. That reason was never true and the harness now exists. `run_pipeline.py` takes an explicit `--instances` path (present since 2026-03-11) and `load_union_graph` receives seven named files as varargs, with no directory scan of any kind. [test_ice_subfamily_trap.py](03_TECHNICAL_CORE/scripts/test_ice_subfamily_trap.py) runs the full pipeline by subprocess against an isolated deliberately-miscategorized fixture (`probe_ice_subfamily_trap.ttl`) and asserts three behaviors: nonzero exit, diagnostics naming both the violated disjoint pair and the trap individual, and failure before certificate emission. It runs in CI on every push and pull request to main.

What remains is breadth rather than infrastructure. Negative coverage is currently two shapes: the fail-closed trap above, and the walk-up verification kiosk (`ARCO_instances_verification.ttl`), which runs clean and is correctly entailed into neither Annex III class. A wider suite of deliberately-miscategorized fixtures, one per gate and per category, is not built.
